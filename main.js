...
function initPlansPage() {
    ...
    console.log('Loaded saved guides from localStorage:', savedGuides.length);
    ...
    // Add event listeners for the Past and Drafts tabs to show "feature under development" message
    const tabButtons = document.querySelectorAll('.tab-btn');
    if (tabButtons) {
        tabButtons.forEach(button => {
            const tab = button.getAttribute('data-tab');
            if (tab === 'past' || tab === 'drafts') {
                button.addEventListener('click', function() {
                    // Show the message
                    showMessage('功能还在开发中');
                    
                    // Still switch tabs as normal for UI consistency
                ...
                button.addEventListener('click', function() {
                    ...
                    document.querySelectorAll('.tab-content').forEach(content => content.classList.remove('active'));
                    const targetTab = document.querySelector(`.tab-content[data-tab-content="${tabName}"]`);
                    if (targetTab) {
                        targetTab.classList.add('active');
                        targetTab.style.display = 'block'; // Ensure visibility
    ...
    if (savedGuides && savedGuides.length > 0) {
        console.log('Processing saved guides:', savedGuides.length);
        savedGuides.forEach(guide => {
            // Extract destination and create start/end dates
            const destination = guide.destination ? guide.destination.split(',')[0] : 'Unknown'; // Just take the city part
            const today = new Date();
            const startDate = new Date(today);
            startDate.setDate(today.getDate() + 30); // Set departure to 30 days from now
            const endDate = new Date(startDate);
            endDate.setDate(startDate.getDate() + (parseInt(guide.duration) || 3)); // Add duration
        ...
        console.log('Added saved guides to upcoming plans:', upcomingPlans.length);
    } else {
        console.log('No saved guides found in localStorage');
    }
    
    // Then add simulated upcoming plans data only if we don't have any real plans
    if (upcomingPlans.length === 0) {
        console.log('Adding sample plans since no saved guides were found');
    ...
    function populatePlans(page) {
        ...
        if (currentPagePlans.length > 0) {
            ...
            console.log('Populated plans container with', currentPagePlans.length, 'plans');
...
function showTravelGuideDetails(guideData) {
    ...
    if (modal && container) {
        ...
        // 生成HTML用于旅行攻略（去掉guide-section guide-destination部分）
        let guideHTML = '';
